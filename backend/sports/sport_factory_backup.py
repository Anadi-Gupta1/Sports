...
class SportFactory:
    ...
    @classmethod
    def _initialize_sports(cls):
        """Initialize all available sport modules"""
        if cls._initialized:
            return
            
        try:
            # Import all available sport modules
            sports_to_load = ["basketball", "tennis", "soccer", "golf"]
            for sport in sports_to_load:
                cls._import_sport_module(sport)
